...
$(document).ready(function () {
  ...
  if (!myroom) {
    // myroom = 1234;
    $('#joinRoom').removeClass('hide');
    // return checkRoomId(myroom);
  } else {
    if (isNaN(myroom)) return alert('Enter a valid roomId');
  ...
  $('#joinRoom button').one('click', function () {
    console.log("---> clicked");
...
function checkRoomIdEnter(field, event) {
  var theCode = event.keyCode ? event.keyCode : event.which ? event.which : event.charCode;
  var roomId;
  if (theCode == 13) {
    console.log("---> RoomId is : ", event.target.value);
